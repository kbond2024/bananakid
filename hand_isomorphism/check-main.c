...
void test_full(hand_indexer_t * indexer) {
  ...
  for(uint64_t i=0; i<permutations; ++i) {
    ...
    for(uint_fast32_t j=0; j<total_cards; ++j) {
      cards[j] = nth_bit(~used, p%(_ISO_CARDS-j)); p /= _ISO_CARDS-j;
      used    |= 1ull<<cards[j];
    } 
...
void test_random(hand_indexer_t * indexer) {
  ...
  uint8_t deck[_ISO_CARDS], pi[_ISO_SUITS];
  for(uint_fast32_t i=0; i<_ISO_CARDS; ++i) {
    deck[i] = i;
  }
  ...
  for(uint64_t i=0; i<10000000; ++i) {
    ...
    for(uint_fast32_t i=0; i<_ISO_SUITS; ++i) {
      uint_fast32_t j = rand()%(_ISO_SUITS-i);
...
int main(int argc, char ** argv) {
  ...
  printf(" ");
  for(uint_fast32_t i=0; i<_ISO_RANKS; ++i) {
    printf("  %c ", RANK_TO_CHAR[_ISO_RANKS-1-i]);
  }
  printf("\n");
  for(uint_fast32_t i=0; i<_ISO_RANKS; ++i) {
    printf("%c", RANK_TO_CHAR[_ISO_RANKS-1-i]);
    for(uint_fast32_t j=0; j<_ISO_RANKS; ++j) {
      cards[0] = deck_make_card(0, _ISO_RANKS-1-j);
      cards[1] = deck_make_card(j<=i, _ISO_RANKS-1-i);
